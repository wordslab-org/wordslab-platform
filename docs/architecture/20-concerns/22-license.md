# License — one policy, four kinds

> **Cross-cutting concern — source of truth: ADR-0022** (which sharpens ADR-0018's license check and ADR-0019's published-thing license as a compliance fact). The ADRs are the source of *detail*; this concern is the organized architecture view — it cites, never restates.

The platform is **100% open source**, but "open" is not one uniform rule. It resolves differently across the four kinds of things the platform touches: **platform components**, **models/implementations**, **vendored OSS UIs**, and **published things**. The through-line: **the platform's own components have a strict open bar; a builder's published things have a permissive-but-expressible policy; and every use must be compliant with the licenses involved.** Nothing is blanket-banned; everything must be used in a way its license permits. (ADR-0022 §1.)

## One policy, four kinds, per-kind rules

The license policy is a **single coherent stance** applied across four kinds of things, with rules that differ per kind because what each thing *is* differs (ADR-0022 §1):
...
## Platform components — three computed tiers, per-tier rules

Component license rules map onto ADR-0018's **computed review-status levels** (`bundled` / `listed` / `third-party` — status is derived from where the code lives, never declared). Each tier has a different bar (ADR-0022 §2):

- **`bundled`** (implemented in the `wordslab-platform` repo itself, ships with the platform): must be **open source**, and its license must keep **the platform repo itself shippable as Apache-2.0** (the org's own license). A **copyleft** component (e.g. GPL-3.0) is therefore acceptable **only if its use does not contaminate the platform's own code** — i.e. it is a **separate process**, never forked, embedded, or linked into the platform's source. (ComfyUI GPL-3.0 behind a thin MIT bridge is fine; embedding/forking it would not be.) This is the strictest bar.
...
**Rejection rule (all tiers of the platform's own stack):** reject components with **no clear license** or with **commercial-use-forbidden / revenue-cap** terms for *software libraries* — a capped library is effectively proprietary at any real scale and a latent trap (see Revenue-threshold clauses). Reject-on-ambiguity: an unclear/unidentifiable license is treated as **not acceptable** until clarified. (ADR-0022 §2.)

**Edge cases** are decided **solo-first by the maintainer/admin** (Laurent), with the rule recorded for contributors — matching ADR-0018's governance. Ambiguity defaults to *not acceptable* until the maintainer rules otherwise. (ADR-0022 §2.)

## Models / implementations — the five-question compliance profile

Model **weight licenses are not OSI software licenses**. Instead of forcing them into the software-license frame, the policy captures each model's terms in a **five-question compliance profile**, recorded per model (in the registry's license tag, ADR-0008/0017; surfaced in `implementation.toml`) (ADR-0022 §3):
...
**Acceptable-to-install rule:** a model is installable if its weights are downloadable and it has a **clear, machine-readable compliance profile**. The **default catalog offers only commercial-safe weights** (Apache/MIT/CC-BY/RAIL-M-commercial — e.g. FLUX.1-schnell, Wan, LTX, Kokoro, CosyVoice2, faster-whisper). **Restricted models** (non-commercial, revenue-capped, research-only — e.g. FLUX-dev, SDXL/SD3.5, XTTS-v2, F5-TTS, HunyuanVideo) remain installable **via explicit user acknowledgment** of their profile. **Non-open-weights models cannot run locally** (they don't have local weights); they can only be reached as cloud implementations, whose terms are part of the same profile. (ADR-0022 §3.)

**Hard-block at the action boundary — everything is possible, but always compliant.** The platform does not just warn; it **blocks** any action a model's license forbids, at the point of use (ADR-0022 §3):
...
Everything *not* forbidden is permitted, subject to the profile (e.g. display-the-model-name where required, question 3). The profile is a **compliance fact** surfaced alongside each model, so a user always knows the terms they are building on — no black box. (ADR-0022 §3.)

## Vendored OSS UIs / heavy dependencies — copyleft as separate process

The platform vendors proven OSS front-ends and engines as **separate processes** (ADR-0002/0017 already treat heavy deps and vendored UIs as separate processes). The stance on **copyleft** vendoring (ADR-0022 §4):

- **Vendoring copyleft is acceptable.** Running copyleft software as its own separate process — communicating over its HTTP API, never forked/embedded/derived into the platform's own code — keeps the platform's Apache-2.0 intact (independent programs talking over normal interfaces are not derivative works).
- **Rules:** (a) never fork/embed the copyleft component's code into the platform repo — it stays a separate process; (b) any **derivative** of it (e.g. custom ComfyUI nodes in `custom_nodes/`) must **stay under the component's copyleft license** — the platform does not ship proprietary custom nodes; (c) **AGPL** candidates are treated like GPL — acceptable as a separate process, but **avoided when a GPL or MIT equivalent achieves the same** (prefer ComfyUI-GPL over A1111/SwarmUI-AGPL); (d) each vendored component's **license + process boundary** is **documented** in the compliance inventory.
...
## Revenue-threshold clauses — accepted for models, rejected for the platform's software libraries

The standing requirement's example ("< $10M/month") comes from the **model** world. The policy sharpens the line (ADR-0022 §5):
...
## Published things — the builder chooses, surfaced as a compliance fact

The two poles meet here: the **platform's own stack is strictly OSS**, but the platform **teaches people to build** and does not force their creations open. The policy parallels ADR-0019's two publishing regimes (ADR-0022 §6):

- **Regime 1 — platform services/implementations** (the "extend the platform" path): inherit the **strict OSS bar** (Platform components). They become platform citizens, so they must satisfy the `bundled`/`listed` component rules — open, clear SPDX, consistent dependencies.
...
**The hard constraint:** a published thing's **actual use must be compliant with its own declared license and any model licenses underneath it** — e.g. you cannot publish an internet demo of an app whose model forbids commercial use (the hard-block above). Declaring a license never waives compliance with the terms underneath. (ADR-0022 §6.)

## Audit kept current

- **Recording:** license is declared as **SPDX** in `service.toml` / `implementation.toml` (ADR-0002/0018) and in the registry's license tag (ADR-0008); the community-catalog listing records carry the license (ADR-0018 §8).
...
- **Keeping it current:** **supply-chain / license scanning** is part of the mechanical quality bar (ADR-0017 §9) with the 1-week freshness gate (ADR-0016/0017) — the audit is kept current by the update/scan machinery, not a one-off review.

## Where the gates land

License gates surface at the action boundary, e.g. the fine-tune gate in Training and Evaluation (30-services/36).
