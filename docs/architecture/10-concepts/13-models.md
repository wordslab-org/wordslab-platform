...
Multiple instances across the fleet are **independent services, not a coordinating cluster** (ADR-0029 §5): each instance serves the models installed on its machine; where a model is deployed is the catalog + install simulation's job (ADR-0004/0005), and the cross-machine "all models" view is a read-only aggregation on the leader core by pull (ADR-0003). Routing to the right instance is the caller's choice — there is no runtime peer-coordination contract between instances.
...
These three are the paradigm pairs that have real swappable engines (Ollama / vLLM / sglang for llm; ComfyUI for diffusion; a classic-ML runtime for ml). Beyond them, the task-shaped model capabilities — **`stt.model`**, **`tts.model`**, **`embeddings.model`**, **`rerank.model`** — are named the same uniform way (every model capability is `<task>.model`, ADR-0029 §1), but they have **no dedicated engine capability of their own**: where a task-shaped model shares a reusable runtime it depends on the generic `ml.engine` (e.g. sentence-transformers is an `ml.engine` implementation shared by embeddings/rerank models), and a self-contained one carries its runtime with no engine dependency (ADR-0029 §2). (The earlier "single-level / self-contained" characterization of these in ADR-0027 §2 is superseded by ADR-0029; the plain task name — `audio.stt`, `audio.tts` — is the *feature* capability on the owning consumer service, while Inference names the model underneath it.)
