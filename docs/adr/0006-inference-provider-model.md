...
> **Evolution note (ADR-0030 — external-model traffic crosses the boundary through the Connectors door):** §1's cloud gateway stays exactly what it is — an **engine implementation** fronting external providers, with the leader-only install, the LiteLLM-inside-a-swappable-engine shape, and the thin platform layer. What ADR-0030 §5 adds is **where its network egress happens**: the gateway's outbound provider calls are **connector-mediated** (the Connectors service's model-access families), so external-model traffic leaves the machine through the platform's **single audited door** — logged, guarded, on one credential path — like every other crossing. Consequence: §2's **provider registry row *is* a connector implementation** (same fields: `id`, `base_url`, `api_key` ref into `secrets`, enabled modalities, privacy tier, bundle flag). The provider registry, bundles (§4), the shared vault (§5) and the two billing modes (§6) are unchanged.
