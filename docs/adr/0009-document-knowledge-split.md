...
> **Evolution note (the §2 "operator subset" framing):** this section's original phrase that Document's bundle-level structural operators form a "limited, specialized subset" of DocETL operators is superseded. ADR-0010 §3 is current: **Document's surface is a set of native representation capabilities** (parse-to-markdown, split+contextualize, extract-raw-fields, embed, retrieve) **— not a subset of DocETL operators**. The bundle-level structural operators (merge/check/format/order) are Document's *generic, purely structural* capabilities — applied to raw data in place, never dependent on a business domain or use case. **All DocETL engine operators live in the Knowledge service**, which executes the reasoning and queries Document's hybrid index where the data lives. Knowledge-computed values ride back into Document's index as stored-but-never-interpreted properties (ADR-0023 §3/§5, storage delegation + bidirectional index). *(Clarified by the #36 consistency pass.)*
