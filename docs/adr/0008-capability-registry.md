...
- **Five typed entries (chosen) vs the original four** — `workflow` is added alongside `tool | agent | skill | data_source` because workflows are composed/invoked/discovered like the rest; models stay out entirely. *(Evolution note: this options entry records ADR-0008's original decision of five types. The registry's entry-type set was later extended to **seven** by ADR-0019 — `tool | api | agent | workflow | skill | data_source | webapp`, `api` and `webapp` added for published things, one entry per exposed surface. The current count is seven; the decision body's "seven typed entries" is authoritative.)*
