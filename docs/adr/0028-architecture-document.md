...
>
> **Evolution note (resolved by #37):** the cross-cutting concern region in §2 gained **`20-concerns/27-v1-scope-and-deferrals.md`** — the consolidated v1-scope ledger ("is this in v1 or later?"), centralizing the v1-limiting/deferral statements already spread across the ADRs as a single checkable pointer table, plus the build-time soft-spot pointers. It cites, never restates; the owning ADR per row stays the source of truth. No new decision is introduced; the graph-query seam (Cypher over Document's Postgres vs recursive CTEs) is recorded there as an open option with the ADR-0023 §3 recursive-CTE default standing in v1 — to be weighed by a later ticket, not settled by #37.
