...
> **Evolution note (ADR-0030 — where the cloud gateway's traffic leaves the machine):** §4 below reclassifies the cloud gateway as the **engine implementation** for cloud models, with each cloud model an implementation that depends on it. That shape stands unchanged. ADR-0030 §5 adds **where the gateway's network egress happens**: its outbound provider calls are **connector-mediated** — they are the Connectors service's **model-access families** (`connectors.llm` / `.diffusion` / `.ml`, whose implementations *are* ADR-0006's provider rows) — so external-model traffic crosses the machine's boundary through the platform's single audited door, logged and guarded like every other crossing. The cloud model → engine dependency declared in §4, the layer-2 engine framing of §3, and the shared service-level machinery of §5 are all unaffected.
