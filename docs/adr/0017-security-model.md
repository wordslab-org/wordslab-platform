...
> **Evolution note (ADR-0030 — the connector consequence tier is declared per tool):** §7.2 below tiers connectors as **wholes** ("read-only connectors (`web` search+fetch, `youtube` watch) are **logged-only**; **send/mutate** connectors (`x`, `github`, `outlook`, `huggingface`) require **human approval**"). ADR-0030 draws that same line — this ADR's own test, *"does data leave the machine / does it send or mutate"* — at the **tool**, which is where a call actually happens: **every tool declares its tier explicitly, with no default**, and a connector is no longer tiered as a whole. Reason: the connectors named send/mutate wholesale all carry read-only operations (`github` reads issues/repos, `outlook` reads mail, `huggingface` searches models), and ADR-0007's `connector-event` trigger ("new email", "new web result") depends on reading being free of approval. Everything else in §7 is unchanged: grants stay **caller-identity-scoped and per-config**, the dashboard still surfaces the per-call confirm for the highest-consequence sends, and harness-native traffic still stays outside the door. The §7 decision text is left as its historical record.
