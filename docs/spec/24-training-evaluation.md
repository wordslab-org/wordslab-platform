...
3. **"Train a small classifier to route my support emails"** (Builder) — fine-tune a classic model / pretrained backbone on a dataset; publish to Inference as a `classic AI` implementation. Supporting: Inference (classic AI), fastai, dataset prep.
